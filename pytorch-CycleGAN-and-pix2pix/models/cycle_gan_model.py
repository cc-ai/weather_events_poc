...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.mapping = {"r": torch.FloatTensor([1, 0]), "s": torch.FloatTensor([0, 1])}

        self.netC_A = networks.define_C(
            self.device, opt.init_type, opt.init_gain, self.gpu_ids
        )
        self.netC_B = networks.define_C(
            self.device, opt.init_type, opt.init_gain, self.gpu_ids
        )
        ...
        if self.isTrain:
            ...
            self.criterionClassifier = torch.nn.MSELoss()
    ...
    def set_input(self, domain, input):
    ...
    def backward_C(self):
        """Calculate classifier loss for domain adaptation"""
        self.loss_C = self.criterionClassifier(
            self.out_C_A, self.mapping[self.domain].unsqueeze(0).to(self.device)
        ) + self.criterionClassifier(
            self.out_C_B, self.mapping[self.domain].unsqueeze(0).to(self.device)
        )
    ...
    def backward_G(self):
        ...
        # MSELoss loss to fool the classifier
        self.loss_G_C_A = self.criterionClassifier(
            self.out_C_A, torch.FloatTensor([[0.5, 0.5]]).to(self.device)
        )
        self.loss_G_C_B = self.criterionClassifier(
            self.out_C_B, torch.FloatTensor([[0.5, 0.5]]).to(self.device)
        )
    ...
    def optimize_parameters(self):
        ...
        self.optimizer_G_E.zero_grad()  # set G_A and G_B's gradients to zero
        self.optimizer_G_D.zero_grad()  # set G_A and G_B's gradients to zero

        self.backward_G()  # calculate gradients for G_A and G_B

        self.optimizer_G_E.step()  # update G_A and G_B's weights
        ...
        self.backward_C()
        self.optimizer_C.step()  # update C
