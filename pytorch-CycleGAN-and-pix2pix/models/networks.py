...
def define_G(
    ...
    return {
        "E": init_net(net_E, init_type, init_gain, gpu_ids),
        "D": init_net(net_D, init_type, init_gain, gpu_ids),
    }


def define_C(
    device, init_type="normal", init_gain=0.02, gpu_ids=[],
):
    C = DAClassifier(proj_dim=128, loss="l2", device=device)
    return init_net(C, init_type, init_gain, gpu_ids)
...
class DAClassifier(nn.Module):
    def __init__(self, proj_dim, loss, device):
    ...
    def setup(self, latent_space):
        self.channels = latent_space.shape[1]
        self.feature_size = latent_space.shape[2]
        self.model = nn.Sequential(
            *[
                Conv2dBlock(self.channels, self.proj_dim, 3, 1, 1).to(self.device),
                nn.MaxPool2d(2).to(self.device),
                BasicBlock(self.proj_dim, int(self.proj_dim / 2), True).to(self.device),
                nn.MaxPool2d(2).to(self.device),
                BasicBlock(int(self.proj_dim / 2), int(self.proj_dim / 4), True).to(
                    self.device
                ),
                nn.AvgPool2d(
                    (int(self.feature_size / 4), int(self.feature_size / 4))
                ).to(self.device),
                Squeeze(-1).to(self.device),
                Squeeze(-1).to(self.device),
                nn.Linear(int(self.proj_dim / 4), 2).to(self.device),
            ]
        )

    def forward(self, x):
        fc_output = self.model(x).to(self.device)
        if (self.loss == "l1") or (self.loss == "l2"):
            fc_output = nn.functional.softmax(fc_output, dim=1)

        return fc_output.to(self.device)
